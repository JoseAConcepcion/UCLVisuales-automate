...
def descargar_archivos(urls, carpeta_destino, extensiones_permitidas):
    global manual
    for url in urls:
        response = requests.get(url)
...
def pedir_hora_inicio():
    while True:
        opcion = input("Selecciona la hora de inicio (ahora mismo (1), a las 01:00 (2), u otra hora en formato 24 horas (ejemplo: 13:30)): ")
        if opcion == '1':
            return 'ahora mismo'
        elif opcion == '2':
            return '01:00'
        elif len(opcion) == 5 and opcion[2] == ':' and opcion[:2].isdigit() and opcion[3:].isdigit():
            return opcion
        else:
            print('Hora no válida. Introduce una hora válida.')
...
hora = pedir_hora_inicio()

descargar_archivos(urls, carpeta_destino, extensiones_permitidas)
print('¡Terminado!')